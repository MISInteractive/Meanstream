...
Namespace Meanstream.Portal.Subscription.Extensibility
...
#Region " Attibute Methods "
        ...
        Private Sub Initialize()
            ...
            If ApplicationId = Nothing Then
                ...
                Throw New InvalidOperationException(String.Format("The Subscriber attribute extensibility infrastructure has not been initialized. MachineName='{0}', AppDomainFriendlyName='{1}', AppBase='{2}'.", machineName, friendlyName, appBase))
            End If

            PortalTrace.WriteLine([String].Concat("Subscriber Extensibity initialized: ", AppFriendlyName, " #", ApplicationId))
        End Sub

        Public Sub Deinitialize()
            PortalTrace.WriteLine([String].Concat("Deinitialize Subscriber Extensibity: ", AppFriendlyName, " #", ApplicationId))
        ...
        Public Function Create(ByVal componentId As Guid, ByVal key As String, ByVal value As Object, ByVal dataType As Type) As Meanstream.Portal.Core.Extensibility.Attribute
            ...
            Dim Atts As Meanstream.Portal.Subscription.Entities.TList(Of Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes) = Meanstream.Portal.Subscription.Data.DataRepository.MeanstreamSubscriptionAttributesProvider.Find("ComponentId=" & componentId.ToString & " AND Name=" & key)
            If Atts.Count > 0 Then
                'attribute exists so return it
                Return Me.GetAttribute(componentId, key)
            End If

            Dim Attribute As Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes = New Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes
            Attribute.DataType = dataType.FullName
            Attribute.Name = key
            Attribute.Value = value
            Attribute.ComponentId = componentId
            Attribute.Id = Guid.NewGuid
            If Meanstream.Portal.Subscription.Data.DataRepository.MeanstreamSubscriptionAttributesProvider.Insert(Attribute) Then
                Return Me.GetAttribute(componentId, key)
            End If
        ...
        Public Function GetAttributes(ByVal ComponentId As Guid) As List(Of Meanstream.Portal.Core.Extensibility.Attribute)
            ...
            Dim Properties As Meanstream.Portal.Subscription.Entities.TList(Of Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes) = Meanstream.Portal.Subscription.Data.DataRepository.MeanstreamSubscriptionAttributesProvider.Find("ComponentId=" & ComponentId.ToString)
            If Properties.Count > 0 Then
                For Each AttributeProperty As Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes In Properties
        ...
        Public Sub Save(ByVal attribute As Meanstream.Portal.Core.Extensibility.Attribute)
            ...
            Dim Atts As Meanstream.Portal.Subscription.Entities.TList(Of Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes) = Meanstream.Portal.Subscription.Data.DataRepository.MeanstreamSubscriptionAttributesProvider.Find("ComponentId=" & attribute.ComponentId.ToString & " AND Name=" & attribute.Key)
            If Atts.Count = 0 Then 'insert
                Dim Att As Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes = New Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes
                Att.Name = attribute.Key
                Att.Value = attribute.Value
                Att.DataType = attribute.DataType.FullName
                Meanstream.Portal.Subscription.Data.DataRepository.MeanstreamSubscriptionAttributesProvider.Insert(Att)
            Else 'update
                Dim Att As Meanstream.Portal.Subscription.Entities.MeanstreamSubscriptionAttributes = Atts(0)
                Att.Name = attribute.Key
                Att.Value = attribute.Value
                Att.DataType = attribute.DataType.FullName
                Meanstream.Portal.Subscription.Data.DataRepository.MeanstreamSubscriptionAttributesProvider.Update(Att)
            End If
        End Sub
        ...
        Public Sub Delete(ByVal attribute As Meanstream.Portal.Core.Extensibility.Attribute)
            ...
            Meanstream.Portal.Subscription.Data.DataRepository.MeanstreamSubscriptionAttributesProvider.Delete(Atts(0))
        End Sub
#End Region
