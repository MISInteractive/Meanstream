...
#Region " Attibute Methods "
        ...
        Private Sub Initialize()
            ...
            If ApplicationId = Nothing Then
                ...
                Throw New InvalidOperationException(String.Format("The attribute extensibility infrastructure has not been initialized. MachineName='{0}', AppDomainFriendlyName='{1}', AppBase='{2}'.", machineName, friendlyName, appBase))
            End If

            PortalTrace.WriteLine([String].Concat("Extensibity initialized: ", AppFriendlyName, " #", ApplicationId))
        End Sub

        Public Sub Deinitialize()
            PortalTrace.WriteLine([String].Concat("Deinitialize Extensibity: ", AppFriendlyName, " #", ApplicationId))
        ...
        Public Function Create(ByVal componentId As Guid, ByVal key As String, ByVal value As Object, ByVal dataType As Type) As Meanstream.Portal.Core.Extensibility.Attribute
            ...
            Dim Atts As Meanstream.Portal.Core.Entities.TList(Of Meanstream.Portal.Core.Entities.MeanstreamAttributes) = Meanstream.Portal.Core.Data.DataRepository.MeanstreamAttributesProvider.Find("ComponentId=" & componentId.ToString & " AND Name=" & key)
            If Atts.Count > 0 Then
                'attribute exists so return it
                Return Me.GetAttribute(componentId, key)
            End If

            Dim Attribute As Meanstream.Portal.Core.Entities.MeanstreamAttributes = New Meanstream.Portal.Core.Entities.MeanstreamAttributes
            Attribute.DataType = dataType.FullName
            Attribute.Name = key
            Attribute.Value = value
            Attribute.ComponentId = componentId
            If Meanstream.Portal.Core.Data.DataRepository.MeanstreamAttributesProvider.Insert(Attribute) Then
                Return Me.GetAttribute(componentId, key)
            End If
        ...
        Public Function GetAttributes(ByVal ComponentId As Guid) As List(Of Meanstream.Portal.Core.Extensibility.Attribute)
            ...
            Dim Properties As Meanstream.Portal.Core.Entities.TList(Of Meanstream.Portal.Core.Entities.MeanstreamAttributes) = Meanstream.Portal.Core.Data.DataRepository.MeanstreamAttributesProvider.Find("ComponentId=" & ComponentId.ToString)
            If Properties.Count > 0 Then
                For Each AttributeProperty As Meanstream.Portal.Core.Entities.MeanstreamAttributes In Properties
        ...
        Public Sub Save(ByVal attribute As Attribute)
            ...
            Dim Atts As Meanstream.Portal.Core.Entities.TList(Of Meanstream.Portal.Core.Entities.MeanstreamAttributes) = Meanstream.Portal.Core.Data.DataRepository.MeanstreamAttributesProvider.Find("ComponentId=" & attribute.ComponentId.ToString & " AND Name=" & attribute.Key)
            If Atts.Count = 0 Then 'insert
                Dim Att As Meanstream.Portal.Core.Entities.MeanstreamAttributes = New Meanstream.Portal.Core.Entities.MeanstreamAttributes
                Att.Name = attribute.Key
                Att.Value = attribute.Value
                Att.DataType = attribute.DataType.FullName
                Meanstream.Portal.Core.Data.DataRepository.MeanstreamAttributesProvider.Insert(Att)
            Else 'update
                Dim Att As Meanstream.Portal.Core.Entities.MeanstreamAttributes = Atts(0)
                Att.Name = attribute.Key
                Att.Value = attribute.Value
                Att.DataType = attribute.DataType.FullName
                Meanstream.Portal.Core.Data.DataRepository.MeanstreamAttributesProvider.Update(Att)
            End If
        End Sub

        Public Sub Delete(ByVal attribute As Attribute)
            If attribute.ComponentId = Nothing Then
                Throw New ArgumentNullException("componentId is required")
            End If
            ...
            Meanstream.Portal.Core.Data.DataRepository.MeanstreamAttributesProvider.Delete(Atts(0))
        End Sub
#End Region
